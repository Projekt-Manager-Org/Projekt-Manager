...
- **Docker Compose** declares the full stack: app, PostgreSQL, Caddy reverse proxy. Single source of truth.
...
- **MinIO** for S3-compatible object storage — **superseded by [ADR-0022](0022-binary-storage-b2-compliance-object-lock.md)**. Deployments use Backblaze B2; MinIO survives only as the dev/CI mirror. The swappability this choice rested on is what made that move a config change.
...
| MinIO (dev/CI mirror only)        | **upstream archived 2026-04-24; frozen, accepted risk** (re-checked 2026-09-12)                                                                   | AGPL-3.0. Pin is terminal — `RELEASE.2025-09-07T16-13-09Z` is the last community image on either registry, and the final source release `RELEASE.2025-10-15T17-29-55Z` was never built into one, so Renovate has nothing to offer. Pulled from **quay.io**: Docker Hub delisted `minio/minio` and `minio/mc` on 2026-09-11 (anonymous pull → 401, repo API → 404), taking CI red until the pin moved registries; quay.io serves the same two tags. A frozen pin is only as durable as the registry hosting it, and one of the two is now gone. Unpatchable at that pin: CVE-2026-40344 / CVE-2026-41145 (HIGH — object write via unsigned-trailer signature bypass) are fixed only in commercial AIStor; CVE-2025-62506 (HIGH — authenticated privesc) is fixed in OSS source we cannot obtain as an image. Mitigated by scope, not by version: dev box and ephemeral CI runners only (the VPS uses B2 per [ADR-0022](0022-binary-storage-b2-compliance-object-lock.md)), no real data, host ports loopback-bound. Migration closed as untargetable; analysis and reopen triggers in [#191](https://github.com/Projekt-Manager-Org/Projekt-Manager/issues/191). The canonical "adopted-already-dying" failure case that motivated [ADR-0027](0027-continuous-dependency-updates-with-supply-chain-scanning.md). |
