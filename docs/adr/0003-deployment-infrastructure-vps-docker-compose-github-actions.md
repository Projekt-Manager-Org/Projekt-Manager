...
| MinIO (dev/CI mirror only)        | **upstream archived 2026-04-24; frozen, accepted risk** (re-checked 2026-09-25)                                                                   | AGPL-3.0. Pin is terminal — `RELEASE.2025-09-07T16-13-09Z` is the last community image ever published; the final source release `RELEASE.2025-10-15T17-29-55Z` never got one. Pulled from **our own GHCR mirror** (`ghcr.io/projekt-manager-org/{minio,mc}`, public, amd64, digest-pinned to upstream), the last published copy — never delete it: Docker Hub delisted the images on 2026-09-11, quay.io followed by 2026-09-25. Not Renovate-tracked; no upstream remains. Source: same tags in the archived `minio/{minio,mc}` GitHub repos. Unpatchable at that pin: CVE-2026-40344 / CVE-2026-41145 (HIGH — object write via unsigned-trailer signature bypass) are fixed only in commercial AIStor; CVE-2025-62506 (HIGH — authenticated privesc) is fixed in OSS source we cannot obtain as an image. Mitigated by scope, not by version: dev box and ephemeral CI runners only (the VPS uses B2 per [ADR-0022](0022-binary-storage-b2-compliance-object-lock.md)), no real data, host ports loopback-bound. Migration closed as untargetable; analysis and reopen triggers in [#191](https://github.com/Projekt-Manager-Org/Projekt-Manager/issues/191). The canonical "adopted-already-dying" failure case that motivated [ADR-0027](0027-continuous-dependency-updates-with-supply-chain-scanning.md). |
