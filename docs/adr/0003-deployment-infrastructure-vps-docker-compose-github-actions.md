...
| MinIO (dev/CI mirror only)        | **upstream archived 2026-04-24; frozen, accepted risk**                                                                                           | AGPL-3.0. `RELEASE.2025-09-07T16-13-09Z` is the last **image** ever published (Docker Hub and Quay both stop there); the final source release `RELEASE.2025-10-15T17-29-55Z` was never built into an image, so the pin is already terminal and Renovate has nothing to offer. Carries unfixable HIGH advisories — CVE-2025-62506 (authenticated privesc) and CVE-2026-40344 / CVE-2026-41145 (unauthenticated object write via unsigned-trailer paths, patched only in commercial AIStor). Mitigated by scope, not by version: MinIO runs only on a dev box and ephemeral CI runners (the VPS uses B2 per [ADR-0022](0022-binary-storage-b2-compliance-object-lock.md)), holds no real data, and its host ports are loopback-bound. Migration closed as untargetable — see [#191](https://github.com/Projekt-Manager-Org/Projekt-Manager/issues/191). The canonical "adopted-already-dying" failure case that motivated [ADR-0027](0027-continuous-dependency-updates-with-supply-chain-scanning.md). |
