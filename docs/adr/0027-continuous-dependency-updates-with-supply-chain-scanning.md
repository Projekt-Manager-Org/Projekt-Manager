...
| More than ~60 global probes per runner IP per hour  | `${{ github.token }}` is an App installation token and GitHub grants Apps no permission on the global `/advisories/{ghsa_id}` endpoint, so those probes run anonymously at 60/h per IP. 24 per run today. Exceeding it exits 2 — loud, not silent. Fix is a scopeless PAT in `secrets` (5000/h); needs a repo admin. |
