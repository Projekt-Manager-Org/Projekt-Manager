...
| A global range **wider** than the publisher's       | The global record is probed only to correct a repo-level match, never to find one — detection against the global DB is OSV-Scanner's job, in the same CI job. A curated range widened before OSV ingests it falls between the two. Keeps the probe at 1 request/run instead of 24, which matters because CI cannot authenticate on that endpoint at all (App tokens hold no permission for it) and anonymous quota is 60/h per IP. |
