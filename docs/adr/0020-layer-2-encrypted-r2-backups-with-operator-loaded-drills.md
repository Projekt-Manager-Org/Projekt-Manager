...
- **Manifest checksum** per table: `SELECT md5(string_agg(md5(row(t.*)::text), '' ORDER BY <pk>)) FROM <table> t`. Deterministic PK ordering is load-bearing — two runs on the same data must produce the same checksum.
- <a id="snapshot"></a>**Manifest and dump share one snapshot.** The manifest transaction exports its snapshot (`pg_export_snapshot()`) and pg_dump imports it (`--snapshot=<id>`). The transaction outlives the dump — an exported snapshot is importable only until its exporting transaction ends. Reading the two from independent snapshots lets an ordinary concurrent write fail Tier 1 on a sound backup, which makes a mismatch unreadable as a corruption signal.
