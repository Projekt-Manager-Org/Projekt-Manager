...
- **The push is in `docker`, not in a second job that re-derives the bytes.** #355's first shape had `publish` rebuild both images from the gha cache and Trivy-scan them again before pushing, on the theory that a warm cache made the rebuild a layer-for-layer hit and the rescan covered the cold case. **Both halves were false**, and the logs of run 33424054809 say so:
  ...
  Fixed at the root rather than tuned: the image is built once and pushed by digest from the same job that scanned and smoked it, so there is no second build to keep honest. `.github/actions/trivy-image` redirects the cache under `$RUNNER_TEMP` and `.dockerignore` carries `.cache` as the durable guard.

  **Attestations and the `docker` exporter.** The validation builds set `provenance: false` and the pushes set `mode=max`. That asymmetry is forced: an attestation makes the build result a manifest list, and the `docker` exporter cannot write one — `ERROR: failed to build: docker exporter does not currently support exporting manifest lists`. GitHub's runners use the classic image store, where that is fatal. It costs nothing: an attestation changes how the final image is assembled, not the cache keys of the steps that build it, so the pushing builds still resolve every layer out of the builder's cache and pay only the upload. Published images keep `mode=max` provenance and `tag-images` copies it forward.

  A note on how that was established, because it is the kind of thing this project's methodology exists for. A first revision of this ADR asserted the opposite — that the attestation asymmetry was itself a cause of the rebuild — on the strength of a local experiment. The experiment was run against a daemon with the **containerd image store** enabled, where `type=docker` accepts a manifest list; GitHub's runners do not, and CI rejected the resulting workflow in 14 seconds. The corrected claim is the one the runner agrees with.

  One shape was rejected on the same evidence: putting all three exporters in a single build. `type=image,push-by-digest` alongside `type=oci` leaves the OCI layout's index referencing a digest the layout does not hold, and the backup image's `FROM` — which resolves through that layout — then fails.
