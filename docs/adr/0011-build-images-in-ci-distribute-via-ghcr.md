...
- **`build-scan-smoke`** — builds both images **once**, Trivy-scans both, boots the stack, runs the smoke, and then pushes both to GHCR with `push-by-digest=true`. A digest-only manifest carries no tag: nothing can `docker pull` it by name, `scripts/deploy.sh` cannot deploy it, and `promote`'s guard 3 cannot find it. Publishing, in the sense that matters, has not happened.
...
- **The push is in `docker`, not in a second job that re-derives the bytes.** #355's first shape had `publish` rebuild both images from the gha cache and Trivy-scan them again before pushing, on the theory that a warm cache made the rebuild a layer-for-layer hit and the rescan covered the cold case. **Both halves were false**, and the logs of run 33424054809 say so:
  ...
  Two causes, both structural rather than tuning problems:

  1. **A `load: true` build and a `push: true` build are different builds.** `docker/build-push-action` adds `--attest type=provenance,mode=max` by itself on a public repo when the build pushes, and cannot add it when the build loads to the runner ("If you're using the `docker` exporter, or you're loading the build results to the runner with `load: true`, no attestations are added to the image" — Docker docs). They therefore never share a solve. The scan ran against the load build's output; the push published the other build's output. `push-images` never scanned what it pushed — the property it existed to provide.
  2. **The scan poisoned the build context.** `trivy-action` defaults `TRIVY_CACHE_DIR` to `$GITHUB_WORKSPACE/.cache/trivy` and unpacks ~1.3 GB there, and `.dockerignore` had no `.cache` entry. Every build after a scan therefore saw a different context and could not hit `COPY . .` again.

  Fixed at the root rather than tuned: the image is built once and pushed by digest from the same job that scanned and smoked it, so there is no second build to keep honest. `.github/actions/trivy-image` redirects the cache under `$RUNNER_TEMP` and `.dockerignore` carries `.cache` as the durable guard. Every build in the composite states `provenance: mode=max` explicitly — including the ones that never push — so the digest push is a genuine cache hit costing only the registry upload. Verified empirically before adoption: a `type=docker` + `type=oci` build and a subsequent `type=image,push-by-digest` build with matching attestations resolve entirely from the builder's cache, and `imagetools create` on the resulting digest preserves the OCI index and the attestation manifest.

  One shape was rejected on evidence: putting all three exporters in a single build. `type=image,push-by-digest` alongside `type=oci` leaves the OCI layout's index referencing a digest the layout does not hold, and the backup image's `FROM` — which resolves through that layout — then fails. Two invocations sharing one cache is the working form.

- **`docker` is a PR-triggered job holding `packages: write`.** It checks out and executes PR-authored code — `.github/actions/build-scan-smoke/action.yml`, `Dockerfile`, `Dockerfile.backup` — with a GHCR write token in scope. `publish` already did (#355); this moves the same grant one job earlier, to where the push now happens. **Accepted, and why:** the composite's `push_by_digest` input restricts the push to same-repo head branches and the operator's dispatch, so the actor already holds push access here and could publish the same images by pushing a branch and dispatching the workflow. Fork PRs — the untrusted population — get a read-only `GITHUB_TOKEN` from GitHub regardless of the `permissions:` block, and `push_by_digest` is false for them anyway. What the grant can produce is also narrower than before: an untagged digest, which nothing deploys until `publish` names it. CONTRIBUTING § Security audit's trigger question reads yes for this change ("communicates externally"), so this is a recorded acceptance rather than an unasked question.
