...
5. Red CI: triage the failure, patch or revert. A red PR assigns itself to the maintainer (`notify-failure` in `ci.yml`) — the wrangler pass is the backstop, not the discovery mechanism. Do not rely on GitHub's Actions email here: it reaches only the run's triggering actor, which on a bot PR is the bot.
