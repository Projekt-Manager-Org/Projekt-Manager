...
5. **Configure branch protection on `main` via a Ruleset.** Classic branch protection is on GitHub's deprecation path — the banner at the top of Settings → Branches points at the replacement. Rulesets are now the single source of truth: Settings → **Rules → Rulesets → New branch ruleset**.
   ...
   - **Rules to enable:**
     ...
     - **Require status checks to pass:** add `lint`, `check`, `docker`, and `publish` (job names — Rulesets match job-level checks, same as classic).
       - **`lint`** is the static-analysis pillar: lockfile-pin drift, allowlist schema, OSV-Scanner (lockfile vulns), Trivy filesystem-secret + IaC scans, compose-reference validation, lint, shellcheck, format, theme-token hygiene, type-check, env-drift, audit-write-path check, build. No DB, no MinIO — runs in parallel with the test pillar.
       ...
       - **`docker`** is the image gate: compose validation, both images built and Trivy-scanned (HIGH/CRITICAL blocks merge), the runtime smoke, and the Tier 1 backup round-trip. It runs on **every** PR and publishes nothing. It used to be path-filtered, which made it report **skipped** on non-image PRs — and a skipped job [counts as a successful required check](https://docs.github.com/en/actions/using-jobs/using-conditions-to-control-job-execution), so requiring it gated nothing for the majority of PRs (#355).
       ...
       - **`publish`** is what orders the merge after the push. It is not an extra image gate — `docker` is that — it is the only thing that makes `promote`'s guard 3 deterministic. Without it, `publish` becomes eligible to start at the exact instant auto-merge does (both wait on `check` + `lint` + `docker`), so on every Renovate auto-merge `main` advances and the head branch is deleted while `publish` is still pushing. It survives today only because `promote` re-runs `check` + `lint` (~4 min) before guard 3 executes while `publish` takes ~1.5 min — a timing margin, not an ordering. Lose the race and guard 3 falls back to the full rebuild, i.e. exactly what #355 exists to remove, on the highest-volume merge path in the repo.

         This reverses the earlier "do NOT add `publish`" note, whose reasoning ("a required check that is legitimately skipped for a whole class of PRs cannot gate anything") contradicted the load-bearing fact one bullet up: [a skipped job reports success](https://docs.github.com/en/actions/using-jobs/using-conditions-to-control-job-execution) and satisfies a required check. Requiring `publish` therefore gates non-fork PRs — the only population guard 3 cares about — and is a no-op for forks, where its `if:` skips it. The real cost is coupling merge to GHCR availability, which is the honest behaviour: if GHCR is down, `promote` cannot publish either.
   ...
   Without `lint`, `check`, `docker` and `publish` all as required contexts, Renovate's auto-merge bypasses the safety net this ADR adds. `docker` runs on every PR and publishes nothing, so it is a real gate rather than an advisory scan — that was not true while it was path-filtered (#355). `publish` is the ordering constraint, not a gate: it is what stops the merge landing before the artifact `promote` will look for exists. The full gating rationale is in [ADR-0027 §Operational](../adr/0027-continuous-dependency-updates-with-supply-chain-scanning.md#operational).
...
5. Red CI: triage the failure, patch or revert. `.github/workflows/notify.yml` raises the signal — a red PR assigns itself to the maintainer and clears the assignment once green, so **assigned to you = red right now**, not "was red at some point"; a red `main` (post-merge `push` or the nightly scan) opens a `ci-red` issue instead, closed automatically on the next green run. The wrangler pass is the backstop, not the discovery mechanism — its remaining job is the one case that emits no event at all: a run that never queued. Do not rely on GitHub's Actions email here: it reaches only the run's triggering actor, which on a bot-merged PR is the bot, on the post-merge push too.
...
- **No-fix-yet OS-package CVE in a base image** (Alpine `node:22-alpine`, `postgresql17-alpine`, etc., where the upstream distro hasn't shipped a patched build yet): Trivy blocks the `docker` job's image scans on every run because `ignore-unfixed: true` is deliberately not set (per [ADR-0027 §Operational](../adr/0027-continuous-dependency-updates-with-supply-chain-scanning.md#operational); see also [§Allowlist](#allowlist-osv-scanner--trivy) for the schema). The deploy pipeline halts until either Alpine ships the fix or an operator writes a deliberate, time-bounded allowlist entry:
...
| Dev/CI storage mirror    | MinIO `minio`, `mc` (frozen, archived upstream, self-hosted on GHCR)                            | [ADR-0003](../adr/0003-deployment-infrastructure-vps-docker-compose-github-actions.md) § Dep lifecycle health — accepted risk, no scanner gate behind it                                       |
...
| Invoice pipeline         | `@cantoo/pdf-lib`, `xmllint-wasm`, `archiver`                                                   | [ARCHITECTURE.md § Invoices Module](../../ARCHITECTURE.md#dep-lifecycle-health-as-of-2026-05-15)                                                                                               |
