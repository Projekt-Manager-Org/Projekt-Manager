...
| `git push` fails with `.husky/pre-push: npm: not found` (nvm users) | Hooks run under non-interactive `sh`, which reads neither `~/.bashrc` (guarded to interactive shells) nor `~/.zshrc`. Put the nvm block in `~/.config/husky/init.sh` — husky sources it before every hook ([husky docs](https://typicode.github.io/husky/how-to.html)). Per-workstation, not in this repo.                                                |
| `node`/`npm`/`npx` missing in IDE terminals, cron, or agent tooling | Same root cause, different shell: non-interactive zsh reads only `~/.zshenv`, never `~/.zshrc`. Put the nvm block there too (~60ms per spawn; re-sourcing it in `~/.zshrc` is harmless — nvm does not duplicate PATH entries). Claude Code additionally replays a shell snapshot captured at session start, so restart the session after adding the file. |
