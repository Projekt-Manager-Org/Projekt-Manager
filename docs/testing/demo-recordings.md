...
prose walkthrough (human-authored)
        ...
        ▼
demo-*.spec.ts ── demo.step(caption, fn) + visible cursor + seeded data
        │
        ▼  Playwright:  demo (desktop 1400×1200)  ·  demo-mobile (Pixel 7)
   video.webm   (silent, captions burned in)
        │  scripts/demo/encode.mjs  (ffmpeg)
        ▼
   demo.mp4   +   demo.srt   (+ optional demo-reel-<WxH>.mp4)
```

## Run

```bash
npm run demo          # record (desktop + mobile) then encode to MP4 + SRT
...
Output lands beside each recording in `test-results/<folder>/`:
`demo.mp4` (H.264, plays everywhere) and `demo.srt` (caption sidecar).
`--reel` concatenates same-resolution clips into `test-results/demo-reel-<WxH>.mp4`.
...
test('Kanban-Überblick', async ({ page }, testInfo) => {
  const demo = await startDemo(page, testInfo); // BEFORE the first goto
  await page.goto('/');

  await demo.step('Das Kanban-Board zeigt jeden Projektzustand', async () => {
    await expect(page.getByTestId('kanban-board')).toBeVisible();
  });

  await demo.finish(); // writes captions.json next to the video
});
```

- `startDemo(page, testInfo)` injects the visible cursor + caption banner. Call
  it before the first navigation (`addInitScript` only applies to later loads).
...
- `demo.finish()` dumps the caption timeline for the `.srt`.
- Drive the UI through `data-testid`s and the `clickView` helper in
  `nav-helpers.ts`; mobile (Pixel 7, below the 768 px breakpoint) renders the
  real mobile shell, so prefer testids over layout-specific selectors.
