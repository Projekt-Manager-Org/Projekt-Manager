...
**Primary / secondary header grouping.** "Rechnungen", "Benutzer", "Daten", "Aktivität", and "Benachrichtigungen" are secondary: lower-frequency admin / observability surfaces, plus the invoice register — owner / office draft and issue invoices on the project page ([project-detail.md §8.15.11](project-detail.md#81511-invoice)). The header groups them under a "Verwaltung" dropdown when a caller sees two or more; a single secondary entry renders inline (dropdown chrome is reserved for real grouping). Bookkeeper's only secondary entry, "Rechnungen", therefore renders inline.
