...
**Surface placement.** The dialog hosts only the _active_ build — a `pending`/`running` job auto-opens it (including the live `ready → download` transition while the operator watches). Once terminal and the dialog is closed (or re-attached on a reload), the affordance moves **inline** into the Export section (download, or `error_detail`), never an auto-opened modal — so a finished export cannot cover the Import action below it.
...
**The Papierkorb is not exported** ([AC-220](../verification.md#1526-attachments)). The preflight says so — `"Der Papierkorb wird nicht mit exportiert."` — because the operator's model of what a backup contains forms here, not at restore time.
...
1. **File picker + destructive confirmation.** Clicking **"Import"** opens the OS file picker. A full-account restore overwrites the deployment, so the dialog renders a visually distinct confirmation input naming the configured confirmation phrase **[C]**; the start action stays disabled until the typed value matches. Alongside the generic destructive notice the gate carries a **second, explicit** one: the archive holds no `hidden` attachments, so the restore is also a Papierkorb purge ([AC-220](../verification.md#1526-attachments)) — `"Der Papierkorb ist nicht Teil des Archivs: gelöschte Anhänge, die dort noch wiederherstellbar sind, gehen dabei endgültig verloren."` "The existing data is deleted" does not tell an operator that the archive cannot bring the trash back; that gap is the whole point of the second line. Below the mobile-warning breakpoint **[C]** the dialog adds: `"Für Desktop-Nutzung gedacht; Importe können sehr groß sein."` Client-side matching is a UX affordance; the server re-validates the phrase ([api.md §14.2.4](../api.md#1424-unified-data-exchange) — Import job override + confirmation).
...
5. **Done / failure summary.** On `status = 'ready'` the dialog renders the restored per-entity counts; the operator closes it explicitly. On `failed`, the `error_detail` is shown. Per-attachment idempotency means a re-run of a job that failed mid-restore resumes rather than duplicates.
...
**A picked file always starts a new import.** `GET /api/import-jobs` keeps returning the last finished job, so a prior _terminal_ job must never take over a dialog opened with a picked archive — it would render that old summary instead of the confirmation gate, making the restore usable exactly once per deployment. Re-attach (no picked file) still resumes onto a terminal job, per the paragraph above.
