...
3. **Progress.** The dialog tracks the job by refetching `GET /api/export-jobs/:id` on each `data_exchange_job_changed` SSE frame ([api.md §14.2.13](../api.md#14213-realtime-events)), surfacing `status`, files-done / total, bytes-done / total, and the current item. The build runs server-side; closing the dialog does not stop it (the job continues and the resume probe re-attaches). There is no client-side cancel in this version.
...
3. **Server processing.** When the upload completes the job goes `running`; the dialog polls `GET /api/import-jobs/:id` (refetched on the `data_exchange_job_changed` SSE) and surfaces the phase — validate → restore → per-attachment re-encrypt — with files-done / total. A validation failure (schema mismatch, corrupt / tampered archive) terminates the job `failed` **before any write**; the dialog shows `error_detail` and the deployment is untouched.
