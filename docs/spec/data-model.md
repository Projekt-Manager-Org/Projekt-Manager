...
- **Payload shape.** `payload` carries the changed fields only, as `{ before, after }` field-keyed objects — not the full row. For a create, `before` is empty and `after` carries the persisted values for every non-server-managed field. For a delete or purge, `after` is empty. For a state transition, `before` and `after` carry `status` and `statusChangedAt`. A create's `after` may additionally carry a denormalized parent label where a cross-project feed or notification must name the parent (e.g. `attachment:add` snapshots the owning project's label) — frozen rendering metadata, not part of the `{ before, after }` diff contract. Full-row snapshots are deliberately excluded (see [ADR-0021 — Alternatives Considered](../adr/0021-audit-log-and-notifications-single-write-path.md#alternatives-considered)).
...
- **Ancestor label snapshot.** `ancestorEntityLabel` is a nullable text column captured at write time alongside the existing `ancestorEntityType` / `ancestorEntityId` pair ([architecture.md §11.12](architecture.md#1112-audit-ancestor-link)). It carries the parent project's `entityLabel` for child-entity rows (`attachment`, `project_worker`, `invoice`) and for `entityType = 'project'` rows (self-ancestor) duplicates the row's own `entityLabel`. Frozen with the audit row so the activity feed surfaces "_who_ did _what_, _where_" without a runtime JOIN against `projects` (which would lose context once a project is purged). Both-or-neither with `ancestorEntityType` / `ancestorEntityId` — when the ancestor pair is null (top-level entities: `customer`, `user`, `company_profile`, `data_import`), `ancestorEntityLabel` is also null. Display metadata only — not part of the `{ before, after }` diff contract. Pinned by [AC-342](verification.md#1532-activity-dock).
