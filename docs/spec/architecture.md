...
- **Nightly E2E**: the E2E test framework runs on a nightly schedule against the protected branch, and on manual trigger, with the same database + object storage + seed shape as the CI gate. It is deliberately outside the merge gate — a serial suite of that size taxes every PR and blocks merges on flake unrelated to the change — so the schedule is what bounds how long a regression can hide. A failed run raises a tracked issue and retains its report + traces as an artifact; a scheduled suite whose failures are neither notified nor debuggable decays as silently as no suite at all. AC-37 in [verification.md §15.7](verification.md#157-engineering) documents the topology from the acceptance-criteria side.
...
- **Registry retention:** a scheduled sweep bounds how long a built image stays pullable, so the rollback range is finite by design. It must never reap the protected-branch pointer, the rollback window behind it, or the head of an open PR. See [ADR-0011 § Retention](../adr/0011-build-images-in-ci-distribute-via-ghcr.md).
...
**Emitter of `data_exchange_job_changed`.** The `DataExchangeJobService` mutators ([data-model.md §5.18](data-model.md#518-data-exchange-job-entity)) emit one frame post-write on every lifecycle transition (`create` / `markRunning` / `markReady` / `markFailed`) and on a throttled subset of `updateProgress` calls (≈1/s or per-N files — lifecycle transitions always emit; progress is the only throttled class, so a thousand-file job does not produce a thousand frames). The job runner owns the throttle, not a per-call-site hook. The frame is invalidation-only; the Daten-view export / import dialog refetches the active job over [api.md §14.2.4](api.md#1424-unified-data-exchange). This is the one emitter not tied to a single wrapping transaction — a job is a sequence of small autocommitted row updates — but it keeps the post-write discipline (emit after the update resolves, never mid-update).
...
**Object Lock retention.** Each rendered PDF/A-3 is written with its own Compliance Object Lock until write time + `INVOICE_OBJECT_LOCK_DAYS` ([§12.2](#122-company-configurable-settings) `[C]`); the full-account import job applies the same lock when it restores one. The lock is per object, not the bucket default: the bucket-default retention `R` must stay within the trash window `L` ([§11.4](#114-object-storage-module)), so it cannot carry a multi-year horizon. A write whose lock the storage provider rejects fails, and the surrounding transaction rolls back.
...
**Why an evaluator, not a publish at the failure site.** The backup failure IS written at a failure site — but that site is the `backup` container, a separate image ([ADR-0020](../adr/0020-layer-2-encrypted-r2-backups-with-operator-loaded-drills.md)), while the publisher binds its `Database` and dispatcher at `app` startup. A publish from the runner reaches an unbound publisher and is silently dropped. The age-derived states (`backup-stale`, `backup-aging`, `drill-expired`, `drill-stale`, never-run) have no failure site at all — nothing happens at the moment a row goes stale. Both are therefore evaluated in the `app` process over the rows the runner writes; the DB is the cross-process channel.
...
- App name, branding, footer brand line — `BRANDING.appName` names the installation across the application surfaces: the header wordmark and login heading, the served document's title, the PWA manifest, and the Service Worker's fallback notification. Rendered-invoice PDF metadata (`Producer` / `Creator`) still carries a literal — tracked, out of AC-363's scope. `BRANDING.shortName` is the installed app's home-screen label, short enough to survive the launcher's truncation. `BRANDING.footerBrandLine` is the application footer's own brand line — not `company_profile.footerText`, which is invoice prose. Source constants, no env vars; [AC-363](verification.md#1519b-app-shell-identity) holds the shell surfaces to one definition site each
- Shell chrome colors — `BRANDING.shell.themeColor` (browser and installed-app chrome) and `BRANDING.shell.backgroundColor` (the splash behind the icon, before first paint). Source constants, no env vars. Independent of the brand accent below: that one is a light/dark pair under a contrast contract, these are single values on surfaces no stylesheet reaches
...
- Favicon and PWA icon set — neither env var nor source constant: the value _is_ the file. Two square SVG marks under `public/` and the PNG set rasterized from them. A deployment replaces the SVGs and regenerates; the procedure lives with the rest of the branding contract in `src/config/brandingConfig.ts`. Distinct from the brand logo above, which is sized for the header strip and the invoice issuer block
...
- Layer 2 freshness thresholds — age of `lastBackupAt` and `lastDrillAt` at which the owner-facing badge switches to amber and to red ([§11.10](#1110-full-state-backup-layer-2)). Source constant: `BACKUP_THRESHOLDS` in `src/config/backupThresholds.ts`. Defaults, in days: backup amber 2 / red 4, drill amber 14 / red 30. No env var
...
- Rate limit buckets — per-session ceilings defined in `getRateLimit()` (`src/server/config/index.ts`); exceeding returns `429 RATE_LIMITED`. Push-subscription mutations (`POST`, `DELETE`) 20/min and password change 5/min are source constants ([api.md §14.2.10](api.md#14210-push-subscription)). The login bucket is env-backed — env var `LOGIN_RATE_LIMIT_MAX`; unset falls back to an environment-aware default (5/min under `NODE_ENV=production`, 30/min elsewhere)
- Attachment per-file size cap — maximum `sizeBytes` of a single attachment original (default 1 MB); enforced at init validation, pinned via the presigned PUT's signed `Content-Length`, and re-verified at complete ([data-model.md §5.13](data-model.md#513-attachment), [api.md §14.2.11](api.md#14211-attachments)). The derived thumbnail carries its own cap (default 200 KB). Env vars: `ATTACHMENT_PER_FILE_CAP_BYTES`, `ATTACHMENT_THUMB_CAP_BYTES`; unset falls back to the build-time defaults in `src/config/attachmentConfig.ts`
- Attachment bulk-fetch caps — maximum attachment count and summed plaintext byte size per request (default 20 files AND 20 MB); breach rejected with `BULK_LIMIT_EXCEEDED` ([api.md §14.2.11](api.md#14211-attachments), [verification.md AC-216](verification.md#1526-attachments)). Env vars: `ATTACHMENT_BULK_MAX_FILES`, `ATTACHMENT_BULK_MAX_BYTES`
- Takeout staging TTL — age past which the scheduled reaper sweeps a staged full-account archive (a `ready` export's downloadable artifact, or an abandoned / terminal import upload) from the VPS-local staging path (default 24 hours; [api.md §14.2.4](api.md#1424-unified-data-exchange) — Jobs — staging + reaper, [data-model.md §5.18](data-model.md#518-data-exchange-job-entity), [verification.md AC-334](verification.md#1514-data-exchange)). Env var: `TAKEOUT_STAGING_TTL_MINUTES` (minutes; default `1440`)
- Takeout staging directory — VPS-local path where the export/import job stages the plaintext archive (env `TAKEOUT_STAGING_DIR`; default under the OS temp dir for zero-config dev/test). **Must resolve to a disk-backed filesystem in production:** the staged archive is the full-account plaintext superset (every business row + every decrypted attachment + each `passwordHash`), so a multi-GB or repeated takeout on a RAM-backed path (the `app` container mounts `/tmp` as tmpfs) exhausts container memory and OOM-kills the VPS. A boot probe refuses to start when `NODE_ENV=production` and the resolved path is on `tmpfs`/`ramfs` (detected via `statfs(2)` magic, not a path heuristic) — fail-closed per the project principle; out of production it warns only. `docker-compose.yml` defaults it to the disk-backed `takeout-staging` named volume.
- Daten-view mobile-warning breakpoint — viewport width (CSS px) below which the Export and Import dialogs render the large-transfer warning (`"… Downloads können sehr groß sein."` / `"… Importe können sehr groß sein."`). Default 480; non-blocking — the user may proceed regardless ([ui/daten.md §8.11.1](ui/daten.md#8111-export), [§8.11.2](ui/daten.md#8112-import))
...
- Threshold-monitor policy — warn band (default 80% of `STORAGE_QUOTA_GB`), clear-band hysteresis margin (default 2 points, so a standing warning clears at 78%), sweep cadence (default 15 minutes), and re-notify cadence while a condition persists (default 1440 minutes = 24 h). Source constant: `THRESHOLD_MONITOR` in `src/config/thresholdMonitor.ts`. No env var — these are policy, parity with `BACKUP_THRESHOLDS`
...
- Invoice Object Lock retention — `INVOICE_OBJECT_LOCK_DAYS`, the per-object Compliance lock (days from write) on every rendered invoice PDF/A-3. Default `3650` in `.env.production.example`, which covers §147 AO (invoices are kept 8 years from the end of the issue year); `0` in `.env.example` writes no lock, so dev binaries stay disposable. Bounds: integer ≥ 0. Independent of the bucket-default retention `R` and the trash window `L` ([§11.4](#114-object-storage-module)). See [§11.14](#1114-invoice-domain), [ADR-0026](../adr/0026-invoices-immutability-and-zugferd.md).
