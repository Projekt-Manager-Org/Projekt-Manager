#!/usr/bin/env bash
#
# Scenario tests for scripts/check-workflow-service-drift.sh.
#
...
# two cases that must NOT fail, since a check that fires on prose would
# get muted the first time someone edits a comment.
#
# Exits 0 when every case matches its expected code; 1 otherwise.
#
# Usage:
#   bash scripts/__tests__/check-workflow-service-drift.test.sh

set -u

REPO_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
CHECK="$REPO_ROOT/scripts/check-workflow-service-drift.sh"
