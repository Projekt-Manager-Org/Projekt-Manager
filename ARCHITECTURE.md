...
One GitHub Actions workflow (`ci.yml`) produces an image; one operator-run script (`scripts/deploy.sh`) promotes it. There is no separate deploy workflow ([ADR-0012](docs/adr/0012-manual-pull-based-deploy-over-wireguard.md)). Five more workflows run outside the merge gate — housekeeping, scheduled scanning and alerting, not image production: `cache-cleanup.yml` deletes GHA caches scoped to a closed PR's merge ref, `ghcr-retention.yml` reaps GHCR image versions outside the rollback window ([ADR-0011 § Retention](docs/adr/0011-build-images-in-ci-distribute-via-ghcr.md)), `security-scheduled.yml` runs a nightly full-tree OSV-Scanner sweep for retroactive CVEs ([ADR-0027](docs/adr/0027-continuous-dependency-updates-with-supply-chain-scanning.md)), `e2e.yml` runs the Playwright suite nightly on `main`, and `notify.yml` raises the human signal when a monitored one goes red.
...
- **Reconciliation — a test invariant, not a job.** [AC-267](docs/spec/verification.md#1526-attachments) asserts the side-table totals equal `SUM(size_bytes + COALESCE(thumb_size_bytes, 0)) GROUP BY (project_id, status)` over `attachments` (and the ciphertext analog) at every checkpoint of the transition matrix, against the real baseline DDL. The scheduled form of the same comparison was weighed at #172 and dropped: it adds coverage only for a trigger disabled at runtime or operator-applied direct SQL, and `StorageUsageService.getProjectUsage` already refuses to serve on a missing side-table row rather than fabricating zeros. Bucket-side byte reconciliation was dropped with it — `pruneBucketOrphans` (#169) already makes the same listing pass and reconciles set membership, which is where the fault classes with a real cause are.
...
- **Documentation drift guards.** Seven checks in the `lint` job plus one in the test job, each failing the build rather than warning. Three _generate_ a published artifact from the code that owns it and fail on drift: the role-permission matrix (AC-343, `scripts/generate-permissions-doc.ts`), the per-role nav matrix (AC-349, `scripts/generate-nav-doc.ts`), and the OpenAPI document (AC-351, `scripts/generate-openapi.ts` — see [§ OpenAPI Document Generation](#openapi-document-generation) for what it does beyond drift). Four _verify_ without generating, because the prose they protect is not derivable: every repository path cited in a code span resolves (AC-347, `scripts/check-doc-paths.sh`); the Module Map and the tree agree in both directions for any directory with a `#### <dir>` subsection (AC-350, `scripts/check-module-map.sh`, with the gated set recorded in `scripts/module-map-gated.txt`); every relative link and `#anchor` resolves (AC-348, `lychee`); every spec §15 AC has a row in `docs/testing/traceability.md` and vice versa (S-ACTR, `scripts/check-traceability.sh` — a `[crit]` AC with no test reference warns without failing). The eighth verifies from the test job, being a unit test rather than a script: api.md §14.4.1's error-code catalogue against `ERROR_CODES` (AC-354, `src/server/__tests__/error-codes.test.ts` — see [§ Error-Code Catalogue](#error-code-catalogue)). Note what generation buys and what it does not: a generated table agrees with its source by construction, so the binding assertion stays hand-written — `ROUTE_TABLE` in `src/config/__tests__/routes.test.ts` for the nav matrix.
- **Prettier is enforced over the whole repository**, not an enumerated glob list — `prettier --check .` in `format:check` and a `**/*` → `prettier --write --ignore-unknown` catch-all in lint-staged (Prettier's own documented pre-commit recipe). The previous `src/**` + `**/*.md` scope was a guard with an unstated blind spot: nothing checked `e2e/`, `scripts/` or the root configs, and the absence of complaints read as compliance while 21 files drifted to Prettier's default `printWidth` 80 (#361). A glob list has to be widened by hand for every new top-level directory and fails silently when nobody does; `.` cannot re-open the gap. Prettier 3 already honours `.gitignore` as a default `--ignore-path`, so build output needs no entry and the full check runs in ~6 s. `.prettierignore` therefore holds exactly one thing — tracked files a **tool** owns: `src/server/db/migrations/meta/` is rewritten by every `drizzle-kit generate`, so formatting it would churn on the next schema change. Generated artifacts do _not_ need an entry: the four doc generators call `prettier.format()` with the resolved repo config (#360), so their output is already conformant. Two known frictions, both accepted: a Renovate config-migration PR (`:configMigration`, via `config:best-practices`) rewrites `.github/renovate.json` in its own JSON style and will fail `format:check` until `npm run format` runs; and lint-staged's globs now overlap, which is why `.husky/pre-commit` passes `--concurrent false` and why declaration order in `package.json` is load-bearing.
