...
- **`check-shard`** (every event, matrix `shard: [1, 2]`) — test pillar, sharded across runners to halve wall-clock. Each shard owns its own Postgres service + in-job MinIO container and runs `npm run test -- --shard=N/2` (no `--coverage`: the report had no uploader and no gate reading it, so the instrumentation was pure cost — `npm run test:coverage` stays available locally). Per-fork DB isolation in `src/test/integration-setup.ts` prevents cross-fork collisions; cross-shard collisions are impossible (separate runners = separate services). Playwright is **not** part of the push/PR gate — `.github/workflows/e2e.yml` runs it nightly on `main` (03:30 UTC) and on `workflow_dispatch`, so CI green on push does not imply E2E green; it implies E2E was green at most 24 h ago. Kept off the merge path deliberately: a serial ~130-test suite costing 4-7 min taxes every PR and would block merges on flake that is not the PR's fault. Failures route through `Notify` (below) as a `ci-red` issue, and the run uploads its Playwright report + traces on failure — without that artifact a nightly is unactionable and gets muted. See [docs/spec/architecture.md §11.7](docs/spec/architecture.md#117-continuous-delivery-pipeline) and [docs/spec/verification.md AC-37](docs/spec/verification.md#157-engineering).
...
`E2E` **is** monitored, as of #377. It was previously excluded on the grounds that it is flaky, `workflow_dispatch`-only, and outside the merge gate — wiring it in would manufacture alert fatigue. The nightly schedule inverts that reasoning: on `main` the failure mode is no longer noise but silence. Twelve runs in the project's lifetime, and the suite sat red for ~3.5 weeks with nothing to say so. The fatigue argument survives against a _dispatch_ suite, not against a scheduled one, and the one-issue-per-incident design above (comment on repeat, auto-close on green) is what bounds it. Known residual gaps, both documented in the workflow header: a run that never queues emits no event at all (only a cron watchdog would catch it — `docs/ops/dep-management.md` § Weekly wrangler is the standing backstop), and `workflow_run` does **not** fire when a monitored workflow fails to start. The second is measured, not assumed — two probes on #349 broke `ci.yml` deliberately (unparseable, then valid YAML with an invalid schema) and neither produced a `Notify` run; the name filter is not the cause, since the workflow entity keeps its registered name. It is left uncovered deliberately rather than worked around, because the case cannot reach `main`: `lint` runs actionlint over `.github/workflows/`, and a PR whose `ci.yml` does not parse reports **no** checks at all, so the required `lint`/`check`/`docker` are never met and it sits blocked.
