...
Two things this deliberately does not publish. **Permission keys do not ride in the requirement array** — scopes are meaningful only on `oauth2` / `openIdConnect` schemes, so `{"sessionCookie": ["project:read"]}` would be valid syntax asserting nothing; the permission dimension stays in § API Surface until #282 decides how to represent it. And the match is checked **only doc → route**: `@fastify/swagger` drops HEAD routes and `@fastify/cors` hides its `OPTIONS *` preflight, so registered routes legitimately outnumber published operations. An operation the generator cannot trace back to a route fails the build (exit 2) rather than publishing with no requirement at all, because that direction advertises a protected endpoint as public.
