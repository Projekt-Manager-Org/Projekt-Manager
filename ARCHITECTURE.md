...
- `logoAsset.ts` — reads the deploy-time brand logo (`BRANDING.mark.logo`) off disk so the header and the rendered PDF are fed by the same file (#189). Checks `public/` then `dist/`, so each environment reads the root the browser is served from: production carries no `public/` and falls through to the build output, while in development this ordering stops a stale `dist/` shadowing the file Vite serves. Confines the configured path to a `brand/` subdirectory of whichever root it resolves under, sniffs PNG / JPEG from magic bytes rather than the extension, and caps the size. Every refusal returns no asset instead of throwing — it runs inside the issuance transaction holding the number-sequence lock, so a branding typo must not be able to abort an issuance.
