...
- **`docker`** (every PR, merge_group, workflow_dispatch) — the pre-merge image gate. Validates `docker compose config` against five combinations (prod; prod+minio; prod+minio+dev; prod with the `backup` profile; dev overlays with the `backup` profile), then runs the `build-scan-smoke` composite: builds **both** images with layer caching, Trivy-scans each (HIGH/CRITICAL), and boots the stack for a runtime smoke — `compose up` against a scratch DB, all four croner registrations, one `node backup-runner.js run` cycle, `load-drill-key` reachability. Catches runtime defects (env-schema drift, `listen_addresses` quoting, ephemeral-pg spawn under the postgres UID) that "does it compile?" misses. Closes with the Tier 1 backup round-trip against the real Postgres binaries (#301). Publishes nothing, and holds no `packages: write` to do it with. The backup image's `FROM` resolves to an OCI layout exported from the app build via buildx `build-contexts`, so both images are gated pre-merge without any GHCR tag existing yet (#219). Deliberately **not** path-filtered: a filter skips the job on PRs that change image bytes through `src/**`, and a skipped required check counts as met (#355).
- **`publish`** (non-fork PRs + workflow_dispatch, `needs: [check, lint, docker]`) — runs the `push-images` composite, pushing **both** images to GHCR tagged `sha-<pr-tip>` and `<branch-slug>`. Every layer is a gha cache hit from `docker`, so this re-materialises rather than rebuilds; the backup push is sequential because its `FROM` references the just-pushed app tag. The `needs` list is the safety property: nothing reaches GHCR from a commit that fails tests, lint, static analysis, an image scan, or the smoke. Fork PRs get no `packages: write` and are skipped — validated but not published, so their merge takes `promote`'s fallback. Production images are built in CI, never on the VPS ([ADR-0011](docs/adr/0011-build-images-in-ci-distribute-via-ghcr.md)).
- **`promote`** (push to `main`) — re-tags the `sha-<pr-tip>` image `publish` produced on the PR as `main`'s SHA via `buildx imagetools create` (no rebuild, ~30s), after verifying the merge traces to a PR, its tree matches the PR tip, and the image exists on GHCR. Falls back to `build-scan-smoke` then `push-images` — in that order — on any guard failure (direct push, `main` advancing between the last green run and the merge, fork PR, force-push after the last build). Build once, promote on merge — see [ADR-0011](docs/adr/0011-build-images-in-ci-distribute-via-ghcr.md).
